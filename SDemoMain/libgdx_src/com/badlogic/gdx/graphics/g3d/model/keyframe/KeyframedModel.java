...
public class KeyframedModel implements AnimatedModel, Disposable {
	...
	protected KeyframedAnimation[] animations;

	public KeyframedModel(KeyframedSubMesh[] subMeshes) {
		this.subMeshes = subMeshes;

		Array<KeyframedAnimation> meshAnims = subMeshes[0].animations.values()
				.toArray();
	...
	private void checkValidity() {
		for (int i = 0; i < subMeshes.length; i++) {
			if (subMeshes[i].animations.size != animations.length)
				throw new GdxRuntimeException(
						"number of animations in subMesh[0] is not the same in subMesh["
								+ i
		...
		for (int i = 0; i < animations.length; i++) {
			...
			for (int j = 0; j < subMeshes.length; j++) {
				KeyframedAnimation otherAnim = subMeshes[j].animations
						.get(anim.name);
				if (otherAnim == null)
					throw new GdxRuntimeException("animation '" + anim.name
							+ "' missing in subMesh[" + j + "]");
	...
	public void setAnimation(String animation, float time, boolean loop) {
		...
		for (int i = 0; i < len; i++) {
			...
			if (anim == null)
				throw new IllegalArgumentException("No animation with name '"
						+ animation + "' in submesh #" + i);
			if (time < 0 || time > anim.totalDuration)
				throw new IllegalArgumentException(
						"time must be 0 <= time <= animation duration");

			final int startIndex = (int) Math
					.floor((time / anim.frameDuration));
			final Keyframe startFrame = anim.keyframes[startIndex];
			final Keyframe endFrame = anim.keyframes[anim.keyframes.length - 1 == startIndex ? loop ? 0
					: startIndex
					: startIndex + 1];
			...
			if (startFrame == endFrame) {
				...
				float alpha = (time - (startIndex * anim.frameDuration))
						/ anim.frameDuration;
				for (int srcIdx = 0, dstIdx = 0; srcIdx < srcLen; dstIdx += dstInc) {
					for (int j = 0; j < numComponents; j++) {
	...
	public KeyframedAnimation[] getAnimations() {
		KeyframedAnimation[] animations = new KeyframedAnimation[subMeshes[0].animations.size];
		int i = 0;
		// subMeshes[0].animations
		for (KeyframedAnimation animation : subMeshes[0].animations.values()) {
	...
	// Add by Gaogao
	public void setAnimation(List<KeyframedAnimation> animations) {
		ObjectMap<String, KeyframedAnimation> map = this.subMeshes[0].animations;
		Keys<String> keys = map.keys();
		for (String key : keys) {
			map.remove(key);
		}

		for (KeyframedAnimation ani : animations) {
			this.subMeshes[0].animations.put(ani.name, ani);
		}
	}
}